...
| CT-028 | **CARRINHO** | P      | Cadastrar carrinho.                      | POST   | /carrinhos                 | 201  | "Cadastro realizado com sucesso" + ID                                             | 
| CT-029 | **CARRINHO** | N      | Cadastrar carrinho com produto inválido. | POST   | /carrinhos                 | 400  | *1                                                                                | 
| CT-030 | **CARRINHO** | N      | Cadastrar carrinho com token inválido.   | POST   | /carrinhos                 | 401  | "Token de acesso ausente, inválido, expirado ou usuário do token não existe mais" | 
| CT-031 | **CARRINHO** | P      | Finalizar compra.                        | DELETE | /carrinhos/concluir-compra | 200  | "Registro excluído com sucesso / Não foi encontrado carrinho para esse usuário"   | 
| CT-032 | **CARRINHO** | N      | Finalizar compra com token inválido.     | DELETE | /carrinhos/concluir-compra | 401  | "Token de acesso ausente, inválido, expirado ou usuário do token não existe mais" | 
| CT-033 | **CARRINHO** | P      | Cancelar compra.                         | DELETE | /carrinhos/cancelar-compra | 200  | "Registro excluído com sucesso / Não foi encontrado carrinho para esse usuário"   | 
| CT-034 | **CARRINHO** | N      | Cancelar compra com token inválido.      | DELETE | /carrinhos/cancelar-compra | 401  | "Token de acesso ausente, inválido, expirado ou usuário do token não existe mais" | 
| CT-035 | **CARRINHO** | P      | Buscar carrinho por ID.                  | GET    | /carrinhos/{_id}           | 200  | JSON com carrinho existente e respectivas informações.                            | 
| CT-036 | **CARRINHO** | N      | Buscar carrinho com ID inválido.         | GET    | /carrinhos/{_id}           | 400  | "Carrinho não encontrado"                                                         | 
| CT-035 | **CARRINHO** | P      | Buscar lista de carrinhos.               | GET    | /carrinhos                 | 200  | JSON com todos os carrinhos existentes e respectivas informações.                 |

* 1: "Não é permitido possuir produto duplicado / Não é permitido ter mais de 1 carrinho / Produto não encontrado / Produto não possui quantidade suficiente"
...
URL Swagger: https://serverest.dev/?lang=pt-BR# <br>
...
Após o clone do projeto, acesse o diretório recém clonado e execute o seguinte comando no terminal: `test`


* Projeto desenvolvido por [Amanda Kopper Hammes](https://github.com/amandahammes) em maio de 2026.